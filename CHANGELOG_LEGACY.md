> ⚠️ This changelog describes the legacy, config-driven BuildStamp implementation.
> It does not apply to BuildStamp Core (API-first) releases starting at v0.1.0.

## [0.9.1] - 2025-11-20
### ✨ Added
- Full **ArgForge v0.6 integration**: commands, subcommands, options, flags.
- New **quiet mode** (`-q`, `--quiet`) across all commands.
- New **verbose mode** (`-v`, `--verbose`) with structured diagnostics.
- Pretty-printed schema output (`buildstamp schema --pretty`).
- Raw JSON inspector (`buildstamp inspect --raw`).
- Unified diagnostic engine (`buildstamp inspect --diagnostic`).
- Prettified output routing allowing shell redirection for CI/CD.
- CLI banner suppressor for machine-readable outputs.
- Complete **EmitCommand** rewrite with:
  - Full validation  
  - Format detection (cs/json/md/text)  
  - Output cleaning  
  - Git/timestamp toggles  
  - Rich verbose telemetry

### 🧠 Changed
- All commands rewritten to use **strict option validation** (per-verb rules).
- Refactored connection between CLI and Core using `BuildInfoEmitter`.
- Harmonized validation pipeline for missing or invalid arguments.
- Standardized output format across inspect, schema, and emit.
- VersionResolver upgraded for nullable safety and robust Git detection.

### 🧰 Fixed
- Quiet mode now correctly suppresses all banners and headers.
- Pretty schema JSON now serializes valid UTF-8 and escapes correctly.
- Emit now validates template/format combinations before executing.
- Zero-color and zero-noise output in quiet mode for CI logs.
- Fixed namespace/class fallback defaults in `BuildInfoEmitter`.
- Correct handling of version override inheritance from config → CLI → environment.

### 📦 Verified
- ✔ `dotnet test` — all tests passing (xUnit).
- ✔ BuildStamp.Cli compiles cleanly with ArgForge architecture.
- ✔ Inspect, schema, and emit validated under:
  - Git context  
  - Non-Git directory  
  - CI context (`CI=true`, GitHub Actions vars)  
- ✔ BuildInfo.cs generated with correct metadata (version, commit, branch, timestamp).
- ✔ Format outputs validated (`cs`, `json`, `md`, `text`).
- ✔ Behavior validated with quiet, verbose, and error conditions.

### 🧹 Maintenance
- Cleaned unused options in ValidateOptions for commands.
- Removed dependency on legacy CLI logic.
- Added internal debug hooks (hidden) for future extension.
- Updated version references and internal docs for 0.9.x refactor phase.

### 🔧 Internal Improvements
- Rebuilt VersionModel with complete SemVer 2.0.0 comparison logic.
- Unified VersionResolver return struct (VersionResolutionResult).
- Stronger nullability guarantees across Core and Cli layers.
- Centralized all printing logic to abide by quiet/verbose contracts.
